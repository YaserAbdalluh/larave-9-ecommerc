        ...
        <form wire:submit.prevent="submit" class="sm:grid-cols-1 sm:grid md:grid md:grid-cols-1">
            ...
            <div class="flex flex-wrap-mx-3 mb-6">
                ...
                {{-- <div class="w-full md:w-1/2 px-3">
                    <x-input label="الصنف" placeholder="اكتب الصنف" wire:model.defer="status" />
                </div> --}}
            </div>
            <div class="flex my-2">
                <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                ...
                {{-- <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                    <x-input label="store" placeholder="" wire:model.defer="store_name" />
                </div> --}}
                <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
            ...
            <div class="flex flex-wrap px-3 -mx-3 mb-6">
                <div class="w-full px-3 py-3">
                    <x-textarea wire:model.defer="description" label="الوصف" placeholder="اكتب الوصف" />
                    <div class="" wire:ignore>
                        {{-- <textarea wire:model.defer="description" label="الوصف" id="summernote" name="editordata">
                        </textarea> --}}
                    </div>
                    @error('description')
            ...
            <x-label class="mb-3" for="">أضف صور</x-label>
            <p class="my-3">الحد الادنى لاضافة صور ,ثلات صور على الاقل.</p>
                ...
                <div class="">
                    <div class="flex">
                        <div x-data="{ isUploading: false, progress: 0 }"
                            ...
                            border-gray-300 border-dashed bg-gray-200 rounded-md space-y-1 hover:opacity-60 cursor-pointer" @click="$refs.fileInput.click()">
                            ...
                            <input x-ref="fileInput" type="file" wire:model.defer="photo" class="hidden" />
                        </div>
                        <div x-data="{ isUploading: false, progress: 0 }"
                            x-on:livewire-upload-start="isUploading = true"
                            x-on:livewire-upload-finish="isUploading = false"
                            ...
                            <input x-ref="fileInput" type="file" wire:model.defer="photo1" class="hidden" />
                        </div>
                        <div x-data="{ isUploading: false, progress: 0 }"
                            x-on:livewire-upload-start="isUploading = true"
                            x-on:livewire-upload-finish="isUploading = false"
                            ...
                            <input x-ref="fileInput" type="file" wire:model.defer="photo3" class="hidden" />
                        </div>
                    </div>
                </div>
                <div>
                    <div class="flex">
                        <div x-data="{ isUploading: false, progress: 0 }"
                            ...
                            <input x-ref="fileInput" type="file" wire:model.defer="photo2" class="hidden" />
                        </div>
                        <div x-data="{ isUploading: false, progress: 0 }"
                            x-on:livewire-upload-start="isUploading = true"
                            x-on:livewire-upload-finish="isUploading = false"
                            ...
                            <input x-ref="fileInput" type="file" wire:model.defer="photo4" class="hidden" />
                        </div>
                        <div x-data="{ isUploading: false, progress: 0 }"
                            x-on:livewire-upload-start="isUploading = true"
                            x-on:livewire-upload-finish="isUploading = false"
                            ...
                            <input x-ref="fileInput" type="file" wire:model.defer="photo5" class="hidden" />
                        </div>
                    </div>
                </div>
            </div>
